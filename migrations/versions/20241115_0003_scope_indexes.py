...
def upgrade() -> None:
    ...
    if not _index_exists(inspector, "orb_policies", "ix_orb_policies_scope_full"):
        op.create_index(
            "ix_orb_policies_scope_full",
            "orb_policies",
            ["module", "submodule", "channel", "subchannel"],
        )
    if not _index_exists(inspector, "orb_policies", "ix_orb_policies_created_at"):
        op.create_index(
            "ix_orb_policies_created_at", "orb_policies", ["created_at"]
        )
    if not _unique_constraint_exists(
        inspector, "orb_policies", "uq_orb_policies_scope_name"
    ):
        with op.batch_alter_table("orb_policies") as batch_op:
    ...
    if not _index_exists(
        inspector, "severity_profiles", "ix_severity_profiles_scope_full"
    ):
        op.create_index(
    ...
    if not _index_exists(
        inspector, "severity_profiles", "ix_severity_profiles_created_at"
    ):
        op.create_index(
    ...
    if not _index_exists(
        inspector, "traditional_runs", "ix_traditional_runs_scope_full"
    ):
        op.create_index(
    ...
    if not _index_exists(
        inspector, "traditional_runs", "ix_traditional_runs_created_at"
    ):
        op.create_index(
    ...
    if not _unique_constraint_exists(
        inspector, "traditional_runs", "uq_traditional_runs_scope_run_id"
    ):
        with op.batch_alter_table("traditional_runs") as batch_op:
            batch_op.create_unique_constraint(
                "uq_traditional_runs_scope_run_id",
                ["module", "submodule", "channel", "subchannel", "run_id"],
            )

    if not _index_exists(inspector, "charts", "ix_charts_scope_full"):
        op.create_index(
            "ix_charts_scope_full",
            "charts",
            ["module", "submodule", "channel", "subchannel"],
        )
    if not _index_exists(inspector, "charts", "ix_charts_dt_utc"):
        op.create_index("ix_charts_dt_utc", "charts", ["dt_utc"])
    if not _index_exists(inspector, "charts", "ix_charts_created_at"):
        op.create_index("ix_charts_created_at", "charts", ["created_at"])
    if not _unique_constraint_exists(
        inspector, "charts", "uq_charts_scope_key"
    ):
    ...
    if not _index_exists(
        inspector, "ruleset_versions", "ix_ruleset_versions_scope_full"
    ):
        op.create_index(
    ...
    if not _index_exists(
        inspector, "ruleset_versions", "ix_ruleset_versions_created_at"
    ):
        op.create_index(
    ...
    if not _unique_constraint_exists(
        inspector, "ruleset_versions", "uq_ruleset_versions_scope_key_version"
    ):
        with op.batch_alter_table("ruleset_versions") as batch_op:
            batch_op.create_unique_constraint(
    ...
    if not _index_exists(inspector, "events", "ix_events_scope_full"):
        op.create_index(
            "ix_events_scope_full",
            "events",
            ["module", "submodule", "channel", "subchannel"],
        )
    if not _index_exists(inspector, "events", "ix_events_event_time_scope"):
        op.create_index(
            "ix_events_event_time_scope",
            "events",
            [event_time_column, "module", "channel"],
        )
    if not _index_exists(inspector, "events", "ix_events_created_at"):
        op.create_index("ix_events_created_at", "events", ["created_at"])
    if not _unique_constraint_exists(
        inspector, "events", "uq_events_scope_key"
    ):
    ...
    if not _index_exists(inspector, "asteroid_meta", "ix_asteroid_meta_scope_full"):
        op.create_index(
    ...
    if not _index_exists(
        inspector, "asteroid_meta", "ix_asteroid_meta_created_at"
    ):
        op.create_index(
    ...
    if not _unique_constraint_exists(
        inspector, "asteroid_meta", "uq_asteroid_meta_scope_designation"
    ):
        with op.batch_alter_table("asteroid_meta") as batch_op:
            batch_op.create_unique_constraint(
                "uq_asteroid_meta_scope_designation",
                ["module", "submodule", "channel", "subchannel", "designation"],
            )

    if not _index_exists(inspector, "export_jobs", "ix_export_jobs_scope_full"):
        op.create_index(
            "ix_export_jobs_scope_full",
            "export_jobs",
            ["module", "submodule", "channel", "subchannel"],
        )
    if not _index_exists(inspector, "export_jobs", "ix_export_jobs_requested_at"):
        op.create_index(
            "ix_export_jobs_requested_at",
            "export_jobs",
            ["requested_at"],
        )
    if not _index_exists(inspector, "export_jobs", "ix_export_jobs_completed_at"):
        op.create_index(
    ...
    if not _unique_constraint_exists(
        inspector, "export_jobs", "uq_export_jobs_scope_key"
    ):
...
def downgrade() -> None:
    with op.batch_alter_table("export_jobs") as batch_op:
        batch_op.drop_constraint("uq_export_jobs_scope_key", type_="unique")
    ...
    with op.batch_alter_table("asteroid_meta") as batch_op:
        batch_op.drop_constraint("uq_asteroid_meta_scope_designation", type_="unique")
    ...
    with op.batch_alter_table("events") as batch_op:
        batch_op.drop_constraint("uq_events_scope_key", type_="unique")
    ...
    with op.batch_alter_table("charts") as batch_op:
        batch_op.drop_constraint("uq_charts_scope_key", type_="unique")
    ...
    with op.batch_alter_table("traditional_runs") as batch_op:
        batch_op.drop_constraint(
            "uq_traditional_runs_scope_run_id", type_="unique"
        )
    ...
    with op.batch_alter_table("orb_policies") as batch_op:
        batch_op.drop_constraint("uq_orb_policies_scope_name", type_="unique")
