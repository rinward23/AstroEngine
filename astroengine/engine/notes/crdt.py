...
class FieldVersion:
    ...
    def from_dict(cls, payload: Mapping[str, str]) -> FieldVersion:
        return cls(
            timestamp=datetime.strptime(payload["timestamp"], ISO_FORMAT).replace(tzinfo=UTC),
...
class CRDTDocument:
    ...
    def apply_patch(self, patch: Mapping[str, object], timestamp: datetime | None = None) -> None:
