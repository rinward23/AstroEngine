# >>> AUTO-GEN BEGIN: AE Decl/Antiscia Detectors v1.0
from __future__ import annotations
from dataclasses import dataclass
from typing import Dict, Iterable, List, Optional

from .utils.angles import norm360, delta_angle, is_within_orb, classify_applying_separating
from .astro.declination import (
    ecl_to_dec, is_parallel, is_contraparallel, antiscia_lon, contra_antiscia_lon,
)
...
def detect_decl_contacts(
    ...
    for t in iso_ticks:
        ...
        if is_parallel(dm, dt, orb_deg_parallel):
            out.append(CoarseHit("decl_parallel", t, moving, target, lm, lt, dm, dt, dm-dt,
                                 classify_applying_separating(lm, pos[moving].get("speed_lon", 0.0), lt)))
        elif is_contraparallel(dm, dt, orb_deg_contra):
            out.append(CoarseHit("decl_contra", t, moving, target, lm, lt, dm, dt, dm+dt,
                                 classify_applying_separating(lm, pos[moving].get("speed_lon", 0.0), lt)))
    return out
...
def detect_antiscia_contacts(
    ...
    for t in iso_ticks:
        ...
        if is_within_orb(d_anti, orb_deg_antiscia):
            out.append(CoarseHit("antiscia", t, moving, target, lm, lt, ecl_to_dec(lm), ecl_to_dec(lt), d_anti,
                                 classify_applying_separating(anti, spd, lt)))
        if is_within_orb(d_contra, orb_deg_contra):
            out.append(CoarseHit("contra_antiscia", t, moving, target, lm, lt, ecl_to_dec(lm), ecl_to_dec(lt), d_contra,
                                 classify_applying_separating(contra, spd, lt)))
    return out
# >>> AUTO-GEN END: AE Decl/Antiscia Detectors v1.0
