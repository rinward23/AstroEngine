...
def test_house_fallback_records_whole_sign(
    monkeypatch: pytest.MonkeyPatch, swiss_ephemeris
) -> None:
    ...
    original = swiss_ephemeris.houses_ex
    ...
    monkeypatch.setattr(swiss_ephemeris, "houses_ex", patched_houses_ex)
...
def test_variant_selection_changes_positions(swiss_ephemeris) -> None:
    ...
    mean_node = mean_adapter.body_position(jd, swiss_ephemeris.MEAN_NODE, body_name="mean_node")
    true_node = true_adapter.body_position(jd, swiss_ephemeris.TRUE_NODE, body_name="true_node")
    south_node = true_adapter.body_position(
        jd, swiss_ephemeris.TRUE_NODE, body_name="south_node"
    )
    ...
    mean_lilith = mean_adapter.body_position(
        jd, swiss_ephemeris.MEAN_APOG, body_name="mean_lilith"
    )
    true_lilith = true_adapter.body_position(
        jd, swiss_ephemeris.OSCU_APOG, body_name="true_lilith"
    )
