...
def test_solar_return_handles_year_boundary() -> None:
    natal = datetime(2000, 1, 1, 0, 0, tzinfo=timezone.utc)
    result = solar_return_datetime(natal, "UTC", 2024)

    assert result.tzinfo is not None
    approx = datetime(2024, natal.month, natal.day, natal.hour, natal.minute, tzinfo=timezone.utc)
    assert _days_between(result, approx) < 3.0


def test_lunar_return_sequence_is_monotonic() -> None:
    natal = datetime(1995, 7, 14, 6, 0, tzinfo=timezone.utc)
    returns = lunar_return_datetimes(natal, n=3, tz="UTC")

    assert len(returns) == 3
    assert returns[0] > natal
    assert all(later > earlier for earlier, later in zip(returns, returns[1:]))
